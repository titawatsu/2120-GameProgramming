...
public class PlayerCollision : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if (_playerCollider.IsTouchingLayers(LayerMask.GetMask("Hazard"))) playerController.TakeDamage();

        if (_playerCollider.IsTouchingLayers(LayerMask.GetMask("Ground"))) playerController.OnFallSound();
        #region Unused
